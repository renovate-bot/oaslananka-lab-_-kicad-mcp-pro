...
- Node.js is only used for Husky local hooks. Use the LTS version in `.node-version`
  and `.nvmrc`, then install with `corepack pnpm install --frozen-lockfile` so `pnpm-lock.yaml`
...
- `corepack pnpm run check:ci` mirrors the regular validation job: metadata check, lint, typecheck, and coverage-gated tests.
- `corepack pnpm run check` is the full local release gate: validation, security audit, and package build.
- `corepack pnpm run security` runs `bandit` and `pip-audit`.
- `uv run pytest --testmon tests/unit/` is useful for local incremental test loops.

## Release Versioning

Use Conventional Commits and let release-please derive the next SemVer version,
update changelog entries, and open the release pull request. If project metadata
changes outside a release PR, regenerate only the metadata mirrors:

```bash
corepack pnpm run metadata:sync
...
- Keep `mcp.json` and `server.json` generated from `pyproject.toml` with `pnpm run metadata:sync`.
